...
### Role-Based Navigation Provider

```csharp
public class RoleBasedNavProvider : INavProvider
{
    private readonly IServiceProvider _serviceProvider;
    private readonly IHttpContextAccessor _httpContextAccessor;
    private readonly IRoleService _roleService;

    public RoleBasedNavProvider(
        IServiceProvider serviceProvider,
        IHttpContextAccessor httpContextAccessor,
    ...
    public async Task<IActionSet> BuildAsync()
    {
        ...
        {
            // Admin navigation
            if (await _roleService.UserHasAnyRoleAsync(user, new[] { "Admin", "SuperAdmin" }))
            {
                await BuildAdminNavigationAsync(builder);
            }

            // Manager navigation
            if (await _roleService.UserHasAnyRoleAsync(user, new[] { "Manager", "Admin" }))
            {
                await BuildManagerNavigationAsync(builder);
            }

            // User navigation (available to all authenticated users)
            await BuildUserNavigationAsync(builder);
        }
        else
        {
            // Anonymous navigation
            await BuildAnonymousNavigationAsync(builder);
        }

        return await builder.BuildAsync();
    }

    private async Task BuildAdminNavigationAsync(ActionSetBuilder builder)
    {
        builder.AddGroup(group => group
            .WithLabel("Administration")
            .WithIcon("fas fa-cog")
            .AddAction(a => a
                .WithLabel("Users")
                .WithIcon("fas fa-users")
                .WithHxGet("/Admin/Users")
                .WithHxPushUrl())
            .AddAction(a => a
                .WithLabel("Roles")
                .WithIcon("fas fa-shield-alt")
                .WithHxGet("/Admin/Roles")
                .WithHxPushUrl())
            .AddAction(a => a
                .WithLabel("System Settings")
                .WithIcon("fas fa-tools")
                .WithHxGet("/Admin/Settings")
                .WithHxPushUrl()));
    }

    private async Task BuildManagerNavigationAsync(ActionSetBuilder builder)
    {
        builder.AddGroup(group => group
            ...
            .AddAction(a => a
                ...
                .WithHxGet("/Manager/Reports")
                .WithHxPushUrl())
            .AddAction(a => a
                .WithLabel("Team")
                .WithIcon("fas fa-user-friends")
                .WithHxGet("/Manager/Team")
                .WithHxPushUrl()));
    }

    private async Task BuildUserNavigationAsync(ActionSetBuilder builder)
    {
    ...
    private async Task BuildAnonymousNavigationAsync(ActionSetBuilder builder)
    {
        builder.AddAction(a => a
            .WithLabel("Home")
